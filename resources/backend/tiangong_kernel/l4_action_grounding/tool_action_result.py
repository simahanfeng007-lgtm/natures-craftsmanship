"""Tool action result objects for L4 phase 4."""

from __future__ import annotations

from dataclasses import dataclass

from tiangong_kernel.l0_primitives.identity import TypedRef

from .identity import L4_ACTION_GROUNDING_SCHEMA_VERSION, ensure_false, ensure_schema_version, ensure_short_text, ensure_true
from .tool_result_envelope import ToolResultEnvelope


@dataclass(frozen=True, slots=True)
class ToolActionResult:
    """Standardized tool action result; it does not write state."""

    result_ref: TypedRef
    request_ref: TypedRef
    tool_result_ref: TypedRef | None = None
    result_envelope: ToolResultEnvelope | None = None
    resource_usage_summary: str = ""
    side_effect_summary: str = "none"
    observation_ref: TypedRef | None = None
    audit_requirement_ref: TypedRef | None = None
    resource_usage_report_ref: TypedRef | None = None
    cost_actual_ref: TypedRef | None = None
    quota_result_ref: TypedRef | None = None
    rate_limit_result_ref: TypedRef | None = None
    dry_run_only: bool = True
    fake_result: bool = False
    real_tool_called: bool = False
    result_only: bool = True
    schema_version: str = L4_ACTION_GROUNDING_SCHEMA_VERSION

    def __post_init__(self) -> None:
        ensure_short_text(self.resource_usage_summary, "ToolActionResult.resource_usage_summary")
        ensure_short_text(self.side_effect_summary, "ToolActionResult.side_effect_summary")
        ensure_false(self.real_tool_called, "ToolActionResult.real_tool_called")
        ensure_true(self.result_only, "ToolActionResult.result_only")
        ensure_schema_version(self.schema_version, "ToolActionResult.schema_version")

    @property
    def has_structured_cost_actual_refs(self) -> bool:
        return self.resource_usage_report_ref is not None and self.cost_actual_ref is not None
